...
if [ ! -f "$HOSTS_DEFAULT_FILE" ]; then
    ...
    add_hosts_base_or_backup_to_file "$HOSTS_DEFAULT_FILE"

    add_domains_to_host "$HOSTS_DEFAULT_FILE" "$DOMAINS_SOCIAL_MEDIA_LIGHT" "Blocked domains: Social media websites (light)"
